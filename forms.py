...
from wtforms.validators import DataRequired, Email, InputRequired, Length, EqualTo, Optional, ValidationError
# Import validators to enforce rules on form fields


def _strong_password(form, field):
    """Require at least 8 characters, one digit, and one uppercase letter."""
    pwd = field.data or ''
    if len(pwd) < 8:
...
class RegisterForm(FlaskForm):
    ...
    password = PasswordField("Password", validators=[InputRequired(), _strong_password, EqualTo('confirm', message='Passwords must match')])
...
# Define a form for admin to respond to user messages
class AdminMessageForm(FlaskForm):
    """Form for admin to respond to user messages."""
    message = TextAreaField('Message', validators=[DataRequired(), Length(min=10, max=5000)])
    # Text area for the message content
    submit = SubmitField('Send Message')
    # Submit button for sending the message
