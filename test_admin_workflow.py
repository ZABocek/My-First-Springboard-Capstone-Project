...
def test_admin_workflow():
    ...
    with app.app_context():
        ...
        test_user = User(
            ...
            is_admin=False,
            is_email_verified=True,  # Required by the login route
        )
        ...
        # --------------- Step 1: log in first --------------------------------
        # The /admin/unlock route now requires an authenticated session.
        ...
        login_extractor = CSRFExtractor()
        login_extractor.feed(login_page.data.decode('utf-8'))
        login_csrf = login_extractor.csrf_token
        assert login_csrf, "No CSRF token found on login page"
        ...
        assert login_response.status_code == 200, \
            "Login failed with status {}".format(login_response.status_code)
